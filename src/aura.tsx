...
function QuickSymptomsSheet({ date, day, showIntimate, onClose, onSave }: { date: string; day: AuraDayEntry; showIntimate: boolean; onClose: () => void; onSave: (patch: Partial<AuraDayEntry>) => void }) {
  ...
  const [showDetails, setShowDetails] = useState(() => day.moods.length > 0 || Boolean(day.energy) || day.intimate === true || day.symptoms.some((symptom) => !frequentIds.has(symptom.id)));
  ...
  return <div className="attention-overlay quick-checkin-overlay" onClick={onClose}>
    <section className="attention-modal quick-checkin-modal" role="dialog" aria-modal="true" aria-labelledby="quick-checkin-title" onClick={(event) => event.stopPropagation()}>
      <div className="attention-handle"/><button className="attention-close" onClick={onClose} aria-label="Закрыть"><X /></button>
      <header className="quick-checkin-header"><span><Heart /></span><div><span className="eyebrow">Симптомы · {formatRuDate(date)}</span><h2 id="quick-checkin-title">Что чувствовали в этот день?</h2></div><em>Сохранится после кнопки</em></header>
