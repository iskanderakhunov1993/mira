...
const nutritionByPhase: Record<CyclePhase, { tip: string; good: string[]; avoid: string[] }> = {
  menstruation: {
    tip: "Добавь продукты с железом и тёплую еду. Мягкий режим может поддержать при спазмах.",
  ...
  luteal: {
    tip: "Тяга к сладкому во второй половине цикла бывает чаще. Попробуй мягкие альтернативы.",
        ...
        <div className="space-y-5">
          ...
          <Card className="border-mira-success/10 bg-[#E0F5E8]/20 p-4">
            ...
            <p className="text-xs text-mira-muted">
              {phase === "menstruation" && "Во время месячных важно не забывать пить. Тёплая вода некоторым помогает при спазмах."}
              ...
              {phase === "luteal" && "Вода может поддержать самочувствие при вздутии, особенно если меньше соли и кофеина вечером."}
            </p>
          </Card>
        </div>
      )}
