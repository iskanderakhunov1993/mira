...
export function buildPersonalizedWorkout(
  ...
  if (highPain) {
    return {
      ...
      duration: 16,
      rationale:
        "Ты отметила сильную боль. Сегодня Mira не предлагает силовую нагрузку: только спокойное дыхание и движения без боли. При новой или усиливающейся боли лучше обратиться к специалисту.",
      signals: [
  ...
  return {
    ...
    rationale: scan
      ...
      : `План опирается на цель «${profile.goal}» и готовность ${readiness}/100. После body scan Mira сможет точнее подобрать фокус и варианты упражнений.`,
    signals: [
      `Цель: ${profile.goal}`,
